...
def _entity_reference_terms(snapshot: dict[str, Any]) -> list[str]:
    """生成用于清理关联实体画像的规范化名称/别名。"""
    candidates = [snapshot.get("name"), *(snapshot.get("aliases") or [])]
    terms: list[str] = []
...
class MemoryGraphRepository:
    ...
    async def delete_entity(self, user_id: str, entity_id: str) -> None:
        ...
        async def _txn(tx):
            ...
            reference_terms = _entity_reference_terms(dict(record))
            context_result = await tx.run(
                cq.ENTITY_SOURCE_CONTEXT,
                user_id=user_id,
                entity_id=entity_id,
            )
            context = dict(await context_result.single() or {})
            statement_ids = [item for item in context.get("statement_ids", []) if item]
            chunk_ids = [item for item in context.get("chunk_ids", []) if item]
            dialogue_ids = [item for item in context.get("dialogue_ids", []) if item]

            await tx.run(
                cq.DELETE_ENTITY_EVENTS,
                user_id=user_id,
                entity_id=entity_id,
            )
            await tx.run(
                cq.DELETE_ENTITY_STATEMENTS,
                user_id=user_id,
                statement_ids=statement_ids,
            )
            await tx.run(
                cq.DELETE_EMPTY_ENTITY_CHUNKS,
                user_id=user_id,
                chunk_ids=chunk_ids,
            )
            await tx.run(
                cq.DELETE_EMPTY_ENTITY_DIALOGUES,
                user_id=user_id,
                dialogue_ids=dialogue_ids,
            )
            await tx.run(
                cq.CLEAN_ENTITY_REFERENCES,
                user_id=user_id,
                entity_id=entity_id,
                reference_terms=reference_terms,
            )
            await tx.run(cq.DELETE_ENTITY, user_id=user_id, entity_id=entity_id)
