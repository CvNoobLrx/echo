...
async def daily_review(user: User = Depends(get_current_user), session: AsyncSession = Depends(get_session)):
    return success(await DailyReviewService(session).get_or_create(user.id))


@router.post("/daily-review/regenerate")
async def regenerate_daily_review(user: User = Depends(get_current_user), session: AsyncSession = Depends(get_session)):
    return success(await DailyReviewService(session).request_regeneration(user.id))
