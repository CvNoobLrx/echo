"""实时会话事件总线 —— 基于 Redis 发布订阅（Pub/Sub）。

对话和研究会话把生成过程中的事件发布到 Redis 频道，SSE 长连接订阅同一频道，
从而支持后台生成、断线重连和跨进程广播。复用现有 Redis（Celery broker），
跨进程/多 worker 天然广播。

事件结构：{"event": "human_message" | "token" | ..., "data": {...}}

另提供「回合锁」：避免同一会话的多个后台生成任务重复执行（同一会话同一时刻
只允许一个 AI 回合在生成）。
"""
import json
...
async def publish(conv_id: str, event: str, data: dict) -> None:
    """向会话频道广播一个事件。失败只记日志，不阻断主流程。"""
    try:
        payload = json.dumps({"event": event, "data": data}, ensure_ascii=False)
        await get_redis().publish(channel_for(conv_id), payload)
    except Exception as e:
        logger.warning("会话事件广播失败: conv=%s event=%s err=%s", conv_id, event, e)


async def open_channel(conv_id: str):
    """打开一个订阅（返回已 subscribe 的 pubsub 对象）。

    本函数把「建立订阅」与「读取消息」拆开，调用方可以先打开频道，再触发会产生事件的
    动作，从而消除「事件早于订阅而漏收」的竞态。
...
async def acquire_turn_lock(conv_id: str) -> bool:
    ...
    try:
        ...
        logger.warning("获取会话回合锁失败: conv=%s err=%s", conv_id, e)
...
async def release_turn_lock(conv_id: str) -> None:
    try:
        ...
        logger.warning("释放会话回合锁失败: conv=%s err=%s", conv_id, e)
