    ...
    async with tracer.trace(
        ...
        async def _retrieve(emit, queries: list[str], start_index: int) -> list[Source]:
            async with tracer.span(
                ...
                span_type="retriever",
                attributes={"echo.retrieval.query_count": len(queries)},
            ) as rsp:
                collected: list[Source] = []
                if ws:
                    provider, api_key = ws
                    try:
                        async with tracer.span("检索:联网", span_type="tool_call", attributes={"echo.tool.name": "web_search", "echo.tool.provider": provider}):
                            collected += await gather_web_sources(provider, api_key, queries, emit=emit)
                    except Exception as e:
                        logger.warning("研究联网检索整体失败（继续）: %s", e)
                try:
                    async with tracer.span("检索:知识库", span_type="tool_call", attributes={"echo.tool.name": "kb_search"}):
                        collected += await gather_kb_sources(session, user_id, queries, kb_ids, emit=emit)
                except Exception as e:
                    logger.warning("研究知识库检索整体失败（继续）: %s", e)
                try:
                    async with tracer.span("检索:MCP 增强", span_type="mcp_call", attributes={"echo.tool.name": "mcp_research"}):
        ...
        async with tracer.span(
            ...
            span_type="writer",
            attributes={"echo.distill.source_count": len(sources)},
        ) as dsp:
        ...
        async with tracer.span(
            ...
            span_type="planner",
            attributes={
                "echo.curator.section_count": len(plan.sections),
                "echo.curator.learning_count": len(learnings),
            },
        ):
