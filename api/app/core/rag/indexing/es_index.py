...
统一索引 echo_chunks（个人版数据量小，单索引 + user_id 过滤足够）。
...
async def ensure_index() -> None:
    """确保 echo_chunks 索引存在且 kb_id 为 keyword 类型。
...
async def _kb_id_type(es) -> str | None:
    """读取 echo_chunks 当前 kb_id 字段类型；不存在返回 None。"""
    try:
...
async def _rebuild_index_fix_kb_id(es) -> None:
    """通过临时索引 reindex，把 echo_chunks 用正确 mapping 重建。
