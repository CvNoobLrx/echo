...
- **新闻晨报**：每天 08:00 根据综合要闻和个人兴趣检索最近 24 小时新闻，由 AI 整理后发送到账号邮箱。
- **工作台**：提供仪表盘、收藏夹、角色与技能配置、工具管理以及 Agent 执行轨迹。
...
| 异步任务 | Celery Worker + Beat，处理后台任务与每日新闻调度 |
| 部署 | Docker Compose、Nginx |

Docker Compose 会运行 PostgreSQL、Elasticsearch、Neo4j、Redis、API、Worker、Beat 和 Web 八个服务。Beat 只负责每天 08:00 触发新闻晨报。原始文件默认保存在 `api/storage`，其余数据保存在 Docker volumes 中。
...
```bash
docker compose -f docker-compose.yml -f docker-compose.prod.yml logs -f api worker beat web
```
